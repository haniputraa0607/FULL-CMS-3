...
@section('page-style')
    ...
    <link href="{{Cdn::asset('kopikenangan-view-asset/public/assets/global/plugins/bootstrap-sweetalert/sweetalert.css') }}" rel="stylesheet" type="text/css" />
...
@section('content')
    ...
    <div class="portlet light portlet-fit bordered">
        <div class="portlet-title">
            <div class="caption">
                <span class="caption-subject font-red sbold uppercase">Detail Transaction Payment Manual</span>
            </div>
        </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 col-sm-12">
                        ...
                        <div class="portlet light portlet-fit portlet-datatable bordered">
                            ...
                            <div class="portlet-body">
                               <label>Name</label>
                               <a href="{{url('user/detail/'.$result['user']['phone'])}}"><h5 style="font-weight: bold;">{{ $result['user']['name'] }}</h5></a>
                            ...
                            <div class="portlet-body row">
                                <div class="col-md-6 col-xs-12">
                                    <label>Date Time</label>
                                        <h5 style="font-weight: bold;">{{ date('d F Y',strtotime($result['transaction_payment_manuals'][0]['payment_date'])) }} {{date('h:i:s',strtotime($result['transaction_payment_manuals'][0]['payment_time']))}}</h5>

                                    <label>Bank</label>
                                    <h5 style="font-weight: bold;">{{ $result['transaction_payment_manuals'][0]['payment_bank'] }}</h5>

                                    <label>Payment Method</label>
                                    <h5 style="font-weight: bold;">{{ $result['transaction_payment_manuals'][0]['payment_method'] }}</h5>

                                    <label>Account Number</label>
                                    <h5 style="font-weight: bold;">{{ $result['transaction_payment_manuals'][0]['payment_account_number'] }}</h5>
                                    
                                    <label>Account Name</label>
                                    <h5 style="font-weight: bold;">{{ $result['transaction_payment_manuals'][0]['payment_account_name'] }}</h5>
                                    
                                    <label>Nominal</label>
                                    <h5 style="font-weight: bold;">Rp {{ number_format($result['transaction_payment_manuals'][0]['payment_nominal']) }}</h5>
                                ...
                                <div class="col-md-6 col-xs-12">
                                    ...
                                    <img style="width:200px" src="{{env('AWS_URL')}}/{{ $result['transaction_payment_manuals'][0]['payment_receipt_image'] }}">
                                    </a>
                                    <br><br>

                                    <label>Note</label>
                                    <h5 style="font-weight: bold;">{{ $result['transaction_payment_manuals'][0]['payment_note'] }}</h5>
                                </div>

                                @if(empty($result['transaction_payment_manuals'][0]['confirmed_at']) && empty($result['transaction_payment_manuals'][0]['cancelled_at']))
                                <div class="col-md-12 col-xs-12" style="text-align:center; margin-top:30px">
                                    ...
                                    <a onClick="hapus('{{$result['transaction_payment_manuals'][0]['id_transaction_payment_manual']}}')" class="btn red"> Decline</a>
                                </div>
                                @endif
                            </div>
                        </div>
                    </div>
                    @if(!empty($result['transaction_payment_manuals'][0]['id_user_confirming']))
                        <div class="col-md-12 col-sm-12">
                            <!-- BEGIN EXAMPLE TABLE PORTLET-->
                            <div class="portlet light portlet-fit portlet-datatable bordered">
                                <div class="portlet-title">
                                ...
                                <div class="portlet-body row">
                                    <div class="col-md-6 col-xs-12">
                                        @if(!empty($result['transaction_payment_manuals'][0]['confirmed_at']))
                                        <label>Accepted At</label>
                                        <h5 style="font-weight: bold;">{{ date('d F Y h:i:s', strtotime($result['transaction_payment_manuals'][0]['confirmed_at'])) }}</h5>
                                        @endif

                                        @if(!empty($result['transaction_payment_manuals'][0]['cancelled_at']))
                                        <label>Declined At</label>
                                        <h5 style="font-weight: bold;">{{ date('d F Y h:i:s', strtotime($result['transaction_payment_manuals'][0]['cancelled_at'])) }}</h5>
                                        @endif
                                    </div>
                                    <div class="col-md-6 col-xs-12">
                                        @if(!empty($result['transaction_payment_manuals'][0]['confirmed_at']))
                                        <label>Accepted By</label>
                                        <h5 style="font-weight: bold;">{{ $result['transaction_payment_manuals'][0]['user']['name'] }}</h5>
                                        @endif
                                    ...
                                    <strong><?php echo $result['transaction_payment_manuals'][0]['payment_note_confirm'] ?></strong>
                                    @endif
                                    </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body form">
                    <input type="hidden" name="id_transaction_payment_manual" value="{{$result['transaction_payment_manuals'][0]['id_transaction_payment_manual']}}">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body" style="text-align:center">
                    <img style="width:80%" src="{{env('AWS_URL')}}/{{ $result['transaction_payment_manuals'][0]['payment_receipt_image'] }}">
                </div> 
            </div>
        </div>
    </div>  
