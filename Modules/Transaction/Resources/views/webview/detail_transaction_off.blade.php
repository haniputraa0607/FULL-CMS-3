    ...
    {{-- @if ($data['transaction_payment_status'] == 'Completed'|| $data['transaction_payment_status'] == 'Paid') --}}
        <div class="kotak">
            <div class="container">
                ...
                @foreach ($data['transaction_payment_offlines'] as $key => $method)
                    <div class="row space-bottom">
                        <div class="col-6 text-black text-12-7px">{{ strtoupper($method['payment_type']) }} - {{ strtoupper($method['payment_bank']) }}</div>
                        <div class="col-6 text-right text-black text-13-3px">IDR {{ str_replace(',', '.', number_format($method['payment_amount'])) }}</div>
                    </div>
                @endforeach
            </div>
        </div>
