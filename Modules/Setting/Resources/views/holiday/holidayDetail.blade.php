...
@section('content')
    ...
    <div class="portlet light portlet-fit bordered">
        <div class="portlet-title">
            <div class="caption">
                <span class="caption-subject font-red sbold uppercase">Holiday Detail</span>
            </div>
        </div>
                        ...
                        <div class="portlet light portlet-fit portlet-datatable bordered">
                            <div class="portlet-title">
                                <div class="caption">
                                    ...
                                    <span class="caption-subject font-green sbold uppercase">Date Holiday</span>
                                </div>
                            </div>
                            <div class="portlet-body">
                               <label>Holiday Name</label>
                               <h4 style="font-weight: bold;">{{ $result['holiday_name'] }}</h4><hr>

                               <label>Holiday Dates</label>
                               @if (!empty($result['date_holiday']))
                                    @foreach($result['date_holiday'] as $holi)
                                        <h4 style="font-weight: bold;">{{ date('d F Y', strtotime($holi['day'])) }}</h4>
                                    @endforeach
                                @endif
                    ...
                    <div class="col-md-6 col-sm-12">
                        ...
                        <div class="portlet light portlet-fit portlet-datatable bordered">
                            <div class="portlet-title">
                                <div class="caption">
                                    <i class=" icon-layers font-green"></i>
                                    <span class="caption-subject font-green sbold uppercase">Outlets</span>
                                </div>
                            </div>
                            <div class="portlet-body">
                               @if (!empty($result['outletHoliday']))
                                    @foreach($result['outletHoliday'] as $row => $holiday)
                                        {{ ++$row.'. '.$holiday['outlet']['outlet_name'] }}<br>
                                    @endforeach
                                @endif
