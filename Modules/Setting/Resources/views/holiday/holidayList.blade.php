...
@section('page-script')
    ...
    <script type="text/javascript">
        ...
        $('#sample_1').on('click', '.delete', function() {
            ...
            $.ajax({
                type : "POST",
                url : "{{ url('setting/holiday/delete') }}",
                data : "_token="+token+"&id_holiday="+id,
                success : function(result) {
                    if (result == "success") {
                        $('#sample_1').DataTable().row(column).remove().draw();
                        toastr.info("Holiday has been deleted.");
                    }
                    else {
                        toastr.warning("Something went wrong. Failed to delete holiday.");
                    }
                }
            });
        });
...
@section('content')
    ...
    <div class="portlet light bordered">
        <div class="portlet-title">
            <div class="caption">
                <span class="caption-subject font-dark sbold uppercase">List Holiday</span>
            </div>
        </div>
                ...
                @if(!empty($result))
                    @foreach($result as $res)
                        <tr>
                            ...
                            <td>{{ date('d F Y', strtotime($res['day'])) }}</td>
                            <td> 
                                <a href="{{ url('setting/holiday/detail', $res['id_holiday']) }}" class="btn blue"><i class="fa fa-search"></i></a> 

                                <a href="{{ url('setting/holiday/edit', $res['id_holiday']) }}" class="btn blue"><i class="fa fa-edit"></i></a> 

                                <a data-toggle="confirmation" data-popout="true" class="btn red delete" data-id="{{ $res['id_holiday'] }}"><i class="fa fa-trash-o"></i></a> 
                            </td>
                        </tr>
                    @endforeach
                @endif
