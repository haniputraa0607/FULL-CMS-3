		...
		<div class="portlet light bordered">
			<div class="portlet-title">
				<div class="caption font-blue ">
					...
					<span class="caption-subject bold uppercase">Campaign Email Outbox List</span>
				</div>
			</div>
			<div class="portlet-body">
				<div class="col-md-12" style="padding-left:0px;padding-right:0px;margin-top:20px;margin-bottom:15px">
					<div class="col-md-6" style="padding: 0px;">
						...
						<input type="text" class="form-control" name="email_sent_subject" placeholder="Search Email Subject" @if(isset($post['email_sent_subject']) && $post['email_sent_subject'] != "") value = "{{$post['email_sent_subject']}}" @endif>
					</div>
					...
					<div class="col-md-1" style="padding: 0px;">
						@if(isset($post['email_sent_subject']) && $post['email_sent_subject'] != "")<a href="{{url('campaign')}}" class="btn red">Reset Search</a>@endif
					</div>
					<div class="col-md-4" style="padding: 0px;">
						<div class="pull-right pagination" style="margin-top: 0px;margin-bottom: 0px;">
							...
							@else
								@if($post['skip'] > 0)
									<li class="page-first"><a href="{{url('campaign')}}/email/outbox/page/{{(($post['skip'] + $post['take'])/$post['take'])-1}}">«</a></li>
								@else
									<li class="page-first disabled"><a href="javascript:void(0)">«</a></li>
								@endif
								
								@if(isset($count) && $count > (($post['skip']+1) * $post['take']))
									<li class="page-last"><a href="{{url('campaign')}}/email/outbox/page/{{(($post['skip'] + $post['take'])/$post['take'])+1}}">»</a></li>
								@else
					...
					<table class="table table-striped table-bordered table-hover">
						...
						<tbody>
							...
							<tr>
								...
								<td>{{$data['email_sent_to']}}</td>
								<td>{{$data['email_sent_subject']}}</td>
								<td>@if($data['email_sent_send_at'] != "")
										{{date('d F Y - H:i', strtotime($data['email_sent_send_at']))}}
									@else
										{{date('d F Y - H:i', strtotime($data['created_at']))}}
									@endif</td>
								<td>
									<a class="btn btn-block green btn-xs" href="{{ url('campaign/email/outbox/detail') }}/{{ $data['id_campaign_email_sent'] }}"><i class="icon-magnifier"></i> Detail </a>
								</td>
							</tr>
							@endforeach
						</tbody>
