...
<a href="{{url('campaign/whatsapp/'.$type)}}" class="btn blue" style="margin-left:15px;margin-bottom:20px">< Back</a><br>
	<div class="col-md-12">
			...
			<div class="portlet-body form">
				<div class="form-body">
					<div class="form-group">
						...
						<input type="text" class="form-control" value="{{$result[0]['whatsapp_'.$type.'_to']}}" readonly> 
					</div>
					<div class="form-group">
						<label>Campaign Title</label>
						<input type="text" class="form-control" value="{{$result[0]['campaign_title']}}" readonly> 
					</div>
					<div class="form-group">
						<label>Send At</label>
						<input type="text" class="form-control" value="{{date('d F Y H:i', strtotime($result[0]['whatsapp_'.$type.'_send_at']))}}" readonly> 
					</div>
					@foreach($result[0]['campaign_whatsapp_'.$type.'_content'] as $index => $content)
					<div class="form-group">
						<label>Content {{$index+1}}</label>
						@if($content['content_type'] == 'text')
						<div class="form-control" readonly style="height: fit-content;">{{$content['content']}}</div>
						@elseif($content['content_type'] == 'image')
							<div class="fileinput-new thumbnail" style="width: fit-content; height: auto; margin-bottom:0">
								<img src="{{$content['content']}}" alt="" style="max-height:190px; max-width:190px">
							</div>
						@elseif($content['content_type'] == 'file')
							@php $file = explode('/', $content['content']) @endphp
							<div class="form-control" readonly style="height: fit-content;"><a href= "{{$content['content']}}"> <i class="fa fa-file-pdf-o"></i> {{end($file)}} </a></div>
						@endif
					</div>
					@endforeach
				</div>
